...
from module.mtcnn.mtcnn import PNet, RNet, ONet
from module.mtcnn.tools import detect_face, get_model_filenames
...
class face_recognition():
    # 静态属性
    model_path = 'model/recognition_model/all_in_one/'
    is_first = True
    p1 = 'null'
    p2 = 'null'
    p3 = 'null'
    ...
    # 获取5个特征点和人脸检测框
    def get_landmarkAndrect(self, img):
        minsize = 20
    ...
    # 获取仿射变换后的图片
    def get_cropImage(self, face_landmarks, img_im, mode=1):
        # 仿射变换
    ...
    # 获取任意图片大小的特征向量
    def get_single_feature_vector(self, img):
        features_points, _ = self.get_landmarkAndrect(img)
        crop_img = self.get_cropImage(features_points[0], img)
    ...
    def init_recog_session():
        if face_recognition.is_first:
            model_path = "model/recognition_model/ckpt_model_d"
            ...
            saver = tf.train.import_meta_graph('model/recognition_model/ckpt_model_d/InsightFace_iter_best_710000.ckpt.meta')
            # 获取当前图，为了后续训练时恢复变量
